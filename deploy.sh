...
step "[2/7] Checking Artifact Registry repository..."
...
step "[3/7] Building and pushing image via Cloud Build..."
...
step "[4/7] Deploying to Cloud Run..."
...
OVERRIDE_KEYS="GCP_PROJECT_ID GCP_LOCATION GCP_SCHEDULER_JOB_ID GCP_GENERATE_JOB_ID SERVICE_URL DRY_RUN"
...
step "[5/7] Configuring send Cloud Scheduler job..."
...
# ══════════════════════════════════════════════════════════════════════════════
# Step 6 — Create or update the daily generate job
# ══════════════════════════════════════════════════════════════════════════════

step "[6/7] Configuring generate Cloud Scheduler job..."

GENERATE_JOB_ID="xoxo-daily-generate"
GENERATE_JOB_URI="${SERVICE_URL}/messages/generate"
GENERATE_JOB_BODY='{"theme":"daily life"}'
GENERATE_DEFAULT_CRON="0 0 * * *"   # midnight America/Sao_Paulo

if gcloud scheduler jobs describe "$GENERATE_JOB_ID" \
   --location="$REGION" \
   --project="$PROJECT_ID" \
   --quiet 2>/dev/null; then
  echo "    Job exists — updating URI and auth headers (schedule unchanged)."
  gcloud scheduler jobs update http "$GENERATE_JOB_ID" \
    --uri="$GENERATE_JOB_URI" \
    --http-method=POST \
    --update-headers="$JOB_HEADERS" \
    --message-body="$GENERATE_JOB_BODY" \
    --location="$REGION" \
    --project="$PROJECT_ID" \
    --quiet
else
  echo "    Job not found — creating with midnight schedule (America/Sao_Paulo)."
  gcloud scheduler jobs create http "$GENERATE_JOB_ID" \
    --schedule="$GENERATE_DEFAULT_CRON" \
    --time-zone="$DEFAULT_TIMEZONE" \
    --uri="$GENERATE_JOB_URI" \
    --http-method=POST \
    --headers="$JOB_HEADERS" \
...
echo "  Send job       : $JOB_ID"
echo "  Generate job   : $GENERATE_JOB_ID (00:00 $DEFAULT_TIMEZONE)"
echo ""
